...
class RsLidarOdometry {
 public:
  ...
  void AddLiDAR(const RsPC::Ptr lidar_cloud, const double timestamp);
  RsPC GetValidCloud() { return Filter_res_.validCloud; };
...
inline void RsLidarOdometry::AddLiDAR(const RsPC::Ptr lidar_cloud, const double timestamp) {
  timestamp_ = timestamp;
  Seg_res_ = image_projection_->cloudHandler(lidar_cloud);
  Filter_res_ = cloud_filter_->CloudHandler(Seg_res_, timestamp_);
}
