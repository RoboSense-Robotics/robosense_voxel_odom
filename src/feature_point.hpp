...
enum LID_TYPE{UNKNOWN, VELODYNE, OUSTER, HESAI, ROBOSENSE, TARTANAIR};
...
public:
  ...
  double blind = 1;
  double blind_far = 300.;
  double omega_l = 3610;

  double process(const RosCloudConstPtr &msg, pcl::PointCloud<PointType> &pl_full)
  {
    double t0 = ROS_TIMESTAMP_TO_SEC(msg->header.stamp);
    switch(lidar_type)
    {
    case VELODYNE:
  ...
  void velodyne_handler(const RosCloudConstPtr &msg, pcl::PointCloud<PointType> &pl_full)
  {
    ...
    if(pl_orig.back().time > 0.01 && pl_orig.back().time < 0.12)
    {
      ...
      for(int i=0; i<plsize; i++)
      {
        ...
        {
          if(ap.x*ap.x + ap.y*ap.y + ap.z*ap.z > blind && ap.x*ap.x + ap.y*ap.y + ap.z*ap.z < blind_far)
          {
  ...
  void ouster_handler(const RosCloudConstPtr &msg, pcl::PointCloud<PointType> &pl_full)
  {
  ...
  void hesai_handler(const RosCloudConstPtr &msg, pcl::PointCloud<PointType> &pl_full)
  { 
    ...
    for(int i=0; i<plsize; i++)
    {
      ...
      if (i % point_filter_num == 0)
      {
        if (added_pt.x*added_pt.x+added_pt.y*added_pt.y+added_pt.z*added_pt.z > blind && added_pt.x*added_pt.x+added_pt.y*added_pt.y+added_pt.z*added_pt.z < blind_far)
        {
  ...
  double robosense_handler(const RosCloudConstPtr &msg, pcl::PointCloud<PointType> &pl_full)
  {
  ...
  void tartanair_handler(const RosCloudConstPtr &msg, pcl::PointCloud<PointType> &pl_full)
  {
